...
namespace DwPhp\Library;
	...
	class systemFunctions{
		...
		public static function enc($p,$c=false){
			$p=(string)$p;$d= utf8_decode('´´´');$l=strlen((string)$p);if($l>1){for($i=0;$i<$l;$i++){$a=ord($p[$i]);$p[$i]=chr($a++);}}else{$a=ord($p);$p=chr($a++);}$p=strrev($p);if($l%2!=0){$p.=$d;}$l=strlen($p);$p=substr($p,$l/2,$l).substr($p,0,$l/2);$p=base64_encode($p);if($c){$p=md5($p);}return $p;
		}
